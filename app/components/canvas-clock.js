...
var CanvasClockComponent = Ember.Component.extend({
  ...
  analogClock: function() {
    ...
    var context  = this.get('canvasCtx'),
	    ...
	    ampm     = timetext.split(" ")[1];

 	context.font = this.get('fontSize')+'px Arial';
 	this.setTimeElements(ampm,timepart);
	this.drawTimeFace();
	this.drawArms();
	this.timeNumbers();
  ...
  drawTimeFace: function() { 
  	var context = this.get('canvasCtx'),
  		...
  		pi      = this.get('pi'),
	    hours   = this.get('currentHr'),
	    dayNite = this.get('dayOrNight');
  	context.beginPath(); 
	context.arc(width/2, height/2, raidus, 0, pi*2, true); 
    context.fillStyle = '#efefef';
	if(this.isDay(dayNite, hours)){
  ...
  drawDot: function() { 
	...
	context.beginPath(); 
	context.arc(width/2, height/2, this.get('armSize'), 0, pi*2, true); 
	context.fillStyle = 'black';
	if(this.isDay(dayNite, hours)){
		context.fillStyle = 'white';
	}
  ...
  drawArms: function() { 
  	this.timeArm(this.get('currentHr')*5 + (this.get('currentMin')/12), 'hour');
    this.timeArm(this.get('currentMin'), 'minute'); 
	this.timeArm(this.get('currentSec'), 'second');
  },

  isDay: function(dayNite, hours) {
